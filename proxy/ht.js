...
mkdir(path.join(__dirname, "../public/img"));
...
function loadSubPage(list) {
  ...
  const pathes = fs.readdirSync(path.join(__dirname, "../public/img/")).map(item => item.split(".")[0]);
...
function loadImg(url, list, name) {
  http.get(url, (res) => {
    ...
    res.on('end', () => {
      try {
        mkdir(path.join(__dirname, "../public/img"));
        fs.writeFileSync(path.join(__dirname, "../public/img/" + name + "." + url.split(".").pop()), rawData, "binary");
        setTimeout(() => {
          console.log("load img success! now start next mission!")
          loadSubPage(list);
        }, Math.random() * 3000 + 500)
